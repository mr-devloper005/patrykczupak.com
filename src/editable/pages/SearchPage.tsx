...
function SearchResultCard({ post }: { post: SitePost }) {
  ...
  return (
    <Link href={href} className="group block overflow-hidden rounded-xl bg-white shadow-[0_18px_50px_rgba(16,10,79,0.09)] transition hover:-translate-y-1 hover:shadow-[0_32px_72px_rgba(16,10,79,0.14)]">
      {image ? (
        <div className="relative h-52 overflow-hidden bg-[var(--slot4-media-bg)] sm:h-56 xl:h-60">
          <img src={image} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
          <span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-[var(--slot4-page-text)]">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-[var(--slot4-page-text)] px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-white">{taskLabel}</span> : null}
        <h2 className="mt-4 line-clamp-3 text-2xl font-extrabold leading-tight text-[var(--slot4-page-text)]">{post.title}</h2>
        {summary ? <p className="mt-4 line-clamp-3 text-sm font-semibold leading-7 text-[var(--slot4-muted-text)]">{summary}</p> : null}
        <span className="mt-5 inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[var(--slot4-accent)]">Open result <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-xl bg-white p-6 shadow-[0_28px_80px_rgba(16,10,79,0.12)] md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-accent)]">{pagesContent.search.hero.badge}</p>
              <h1 className="mt-5 text-5xl font-extrabold leading-tight sm:text-7xl">{pagesContent.search.hero.title}</h1>
            ...
            <form action="/search" className="self-end rounded-xl border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-4 sm:p-5">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                ...
                <select name="task" defaultValue={task} className="rounded-xl border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-extrabold outline-none">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-sm bg-[var(--slot4-accent)] px-6 text-sm font-extrabold text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-extrabold">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
            <Link href="/article" className="inline-flex items-center gap-2 rounded-sm border border-[var(--slot4-accent)] bg-white px-5 py-3 text-sm font-extrabold text-[var(--slot4-accent)]">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-xl border border-dashed border-[var(--editable-border)] bg-white p-10 text-center">
